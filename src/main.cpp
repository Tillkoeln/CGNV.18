...
unsigned int nTargetSpacing = 1 * 62; // 62 seconds
unsigned int nStakeMinAge = 12 * 60 * 60; // 12 hours
unsigned int nStakeMaxAge = 36 * 60 * 60; // 36 hours
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 38;
...
const string strMessageMagic = "KolschCoin Signed Message:\n";
...
{
    int64_t nSubsidy = 0.0404 * COIN;
    
    if (pindexBest->nHeight+1 == 1)
    {
      nSubsidy = 415.18575 * COIN;
      return nSubsidy + nFees;
    }
      
    else if (pindexBest->nHeight+1 <= 41760)
    {
      nSubsidy = 0.0404 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 83520)
    {
      nSubsidy = 0.0202 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 125280)
    {
      nSubsidy = 0.0101 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 167040)
    {
      nSubsidy = 0.00505 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 208800)
    {
      nSubsidy = 0.002525 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 250560)
    {
      nSubsidy = 0.00126250 * COIN;
      return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "KolschCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
